import styles from "@/styles/home/product.module.css";
import Image from "next/image";
import Link from "next/link";

const ProductBox = ({product}) => {
  return (
    <div className={`${styles.card} bg-white p-4 rounded-lg  shadow-lg`}>
      <Link href={"/store/product-details/" + product.product_id}>
        {/* <Image
          width={30}
          height={150}
          src="https://images.pexels.com/photos/3094799/pexels-photo-3094799.jpeg?auto=compress&cs=tinysrgb&dpr=1&w=500"
          alt='SmartImg'
          className={`${styles.productImage} w-full h-48 object-cover rounded-md`}
        /> */}
          {/* <img
        src={product.image}
        alt={product.name}
        className={`${styles.productImage} w-full h-48 object-cover rounded-md`}
         width={30}
         height={150}
        onError={(e) => {
          e.target.src = "https://via.placeholder.com/300x200?text=No+Image"; // Fallback image
        }}
      /> */}
        <Image width={30} height={100} src={product.image} alt={product.name} className={`${styles.productImage} w-full h-48 object-cover rounded-md`} />
      </Link>
      <div className="mt-4">
        <h3 className='text-xl min-h-[70px] font-semibold text-gray-800'>{product.name}</h3>
        <h4 className='text-sm font-semibold text-gray-800'>{product.description}</h4>
        <p className='text-gray-900 font-semibold mt-2'>${product.price}</p>
        <button className="mt-4 bg-pink-500 text-white px-4 py-2 rounded hover:bg-pink-600 transition duration-300">
          Buy Now
        </button>
      </div>
    </div>
  );
};

export default ProductBox;

